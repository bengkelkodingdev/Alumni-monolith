@props(['listingmagang'])
 ...
 <div class="job-listingmagangs-container" style="margin-bottom: 1.25rem; margin-left: 0.5rem;"> 
   <div> 
     <div style="display: flex; align-items: center; border: 1px solid #d1d5db; border-radius: 0.5rem; padding: 1rem; background-color: white; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05);"> 
       ...
       <div style="flex-grow: 1;"> 
         <h3 style="font-size: 1.25rem; font-weight: 600; margin-bottom: 0.25rem;"> 
           {{-- <a href="/loker/{{ $listingmagang->id }}" style="color: inherit; text-decoration: none;">{{ $listingmagang->Posisi }}</a> --}}
           
            {{-- <button type="button" style="background: none; border: none; color: inherit; text-decoration: none; padding: 0; cursor: pointer;"
                    class="btn btn-link" data-bs-toggle="modal" data-bs-target="#dialogShowLoker" data-id="<?php $listingmagang = $loker->id; ?>">
                {{ $listingmagang->Posisi }}
            </button>
            <button type="button" style="background: none; border: none; color: inherit; text-decoration: none; padding: 0; cursor: pointer;"
                    class="btn btn-link" data-bs-toggle="modal" data-bs-target="#dialogShowLoker" data-id="{{ $listingmagang->id }}">
                {{ $listingmagang->Posisi }}
            </button> --}}
       ...
       <div style="display: flex; flex-direction: column; align-items: flex-end;"> 
         <span style="background-color: #d1fae5; color: #065f46; font-size: 0.75rem; font-weight: 600; 
                    ...
                    margin-bottom: 0.5rem;">{{ $listingmagang->TipeMagang }}</span> 
