    ...
    <tbody>
        ...
        <tr class="border-gray-300">
            <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
                <a href="{{ route('loker.show', $listing ->id) }}" style="color: inherit; text-decoration: none;">{{$listing ->NamaPerusahaan}}</a>
            </td>
            <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
              <a href="/loker/{{$listing ->id}}" style="color: inherit; text-decoration: none;"> {{$listing ->Posisi}} </a>
            </td>
            <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
              <button type="info" class="btn btn-custom btn-info me-2 text-white px-3 py-2 rounded-5" style="width: 100px; text-align: center;" data-bs-toggle="modal"
                data-bs-target="#dialogShowLoker" data-id="{{ $listing ->id }}">
                Detail
              </button>
            </td>
            <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
              <button type="submit" class="btn btn-custom btn-primary me-2 text-white px-3 py-2 rounded-5" style="width: 100px; text-align: center;" data-bs-toggle="modal"
                data-bs-target="#dialogEditLoker" data-id="{{ $listing ->id }}">
                Edit
              </button>
            </td>
            ...
            <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
                <form method="POST" action="/loker/{{$listing ->id}}/delete" onsubmit="return confirm('Delete?')">
                    @csrf
                    @method('DELETE')
                    <button  value="{{ $listing ->id }}" type="submit" class="btn btn-danger text-white px-3 py-2 rounded-5" style="width: 100px; text-align: center;">
                      Delete
                    </button>
                </form>
            </td>
...
<!--Dialog Edit Logbook-->
@include('alumni.loker.edit')
<!--Dialog Edit Logbook-->
@include('alumni.loker.show')
@endsection --}}
  ...
  <table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr class="border-gray-300">
            ...
            {{-- <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
              <button type="info" class="btn btn-custom btn-info me-2 text-white px-3 py-2 rounded-5" style="width: 100px; text-align: center;" data-bs-toggle="modal"
                data-bs-target="#dialogShowLoker" data-id="{{ $loker->id }}">
                Detail
              </button>
            </td>
            <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
              <button type="submit" class="btn btn-custom btn-primary me-2 text-white px-3 py-2 rounded-5" style="width: 100px; text-align: center;" data-bs-toggle="modal"
                data-bs-target="#dialogEditLoker" data-id="{{ $loker->id }}">
                Edit
              </button>
            </td> --}}
            {{-- <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
              <button type="info" class="btn btn-custom btn-info me-2 text-white px-3 py-2 rounded-5"
            ...
            <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
              <button type="submit" class="btn btn-custom btn-primary me-2 text-white px-3 py-2 rounded-5"
...
document.addEventListener('DOMContentLoaded', function () {
    var showModalButtons = document.querySelectorAll('[data-bs-target="#dialogShowLoker"]');
    var editModalButtons = document.querySelectorAll('[data-bs-target="#dialogEditLoker"]');

    showModalButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            var listingId = this.getAttribute('data-id');
            fetch(`/loker/${listingId}`)
                .then(response => response.json())
                .then(data => {
                    // Populate the show modal with data
                    document.querySelector('#dialogShowLoker .modal-body').innerHTML = `
                        <p>Nama Perusahaan: ${data.NamaPerusahaan}</p>
    ...
    editModalButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            var listingId = this.getAttribute('data-id');
            fetch(`/loker/${listingId}/edit`)
                .then(response => response.json())
                .then(data => {
                    // Populate the edit modal with data
                    document.querySelector('#dialogEditLoker input[name="NamaPerusahaan"]').value = data.NamaPerusahaan;
                    document.querySelector('#dialogEditLoker input[name="Posisi"]').value = data.Posisi;
                    document.querySelector('#dialogEditLoker textarea[name="Deskripsi"]').value = data.Deskripsi;
                });
        });
    });
});
...
<table class="table table-bordered">
  ...
  <tbody>
      ...
      <tr class="border-gray-300">
          ...
          <td class="px-4 py-8 border-t border-b border-gray-300 text-lg centered-column">
            <button type="button" class="btn btn-custom btn-primary me-2 text-white px-3 py-2 rounded-5"
                style="width: 100px; text-align: center;" data-bs-toggle="modal" data-bs-target="#dialogEditLoker"
